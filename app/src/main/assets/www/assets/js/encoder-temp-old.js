...
function validate_main_form() { // ok
    ...
    if ($("#welec").val() == 1) {
        ...
        $(".SectionElecDetailse").val("").notRequired();
    }
    /**    if ($("#reloc_ind").val() == 1) {
            $(".reloc_tr").show();
    ...
    if ($("#garb_collect").val() == 1) {
        ...
        $(".garb_collect").required();
    } else {
        $("#SectionGarbCollect").hide();
        $(".garb_collect").val("").notRequired();
    }
 ...
 */
    if ($("#cropind").val() == 1) {
        ...
        $(".cropind, .hpq_agriequip").val("").notRequired();
    }
    if ($("#poultind").val() == 1) {
...
function validated_add_mem_form() { // ok
    ...
    if ((age >= 10 && numofw > 0 && numofw != ofws) || (age >= 10 && numofw > 0 && memIsOFW)) {
        $(".ofwind_tr").show();
        $("#ofwind").required();
    } else {
        $(".ofw, .ofwind_tr").hide();
    ...
    if ((age >= 10 && numunipar > 0 && numunipar != unipars) || (age >= 10 && numunipar > 0 && memIsUnipar)) {
        $(".soloparent").show();
        $("#uniparind").required();
    } else {
        $(".soloparent").hide();
    ...
    if ((age >= 12 && $("#sex").val() == 2 && numpreg > 0 && numpreg != pregnants) || (age >= 12 && $("#sex").val() == 2 && numpreg > 0 && memIsPregnant)) {
        $(".sdg").show();
        $(".rcs_inputs").required();
    } else {
        $(".sdg").hide();
    ...
    if ($("#educind").val() == 1) {
        ...
        $("#ynotsch, #ynotsch_o").val("").notRequired();
    ...
    if ($("#gradel").val() >= 23 && $("#gradel").val() <= 41) {
        ...
        $(".course_tr, course_o_tr").hide();
        $("#course, #course_o").val("").notRequired();
    }
    if ($("#psced7").val() == 9999999) {
        $(".course_o_tr").show();
        $("#course_o").required();
    } else {
        $(".course_o_tr").hide();
        $("#course_o").val("").notRequired();
    }
    ...
    if (($("#educal").val() >= 23 && $("#educal").val() <= 41) || ($("#educal").val() >= 210 && $("#educal").val() <= 400)) {
        $(".fcourse_tr, .fcourse_o_tr").show();
        $("#fcourse, #fcourse_o").required();
    } else {
        $(".fcourse_tr").hide();
        $("#fcourse").val("").notRequired();
    }
    if ($("#fpsced7").val() == 9999999) {
        $(".fcourse_o_tr").show();
        $("#fcourse_o").required();
    } else {
        $(".fcourse_o_tr").hide();
        $("#fcourse_o").val("").notRequired();
    }
    ...
    if ($("#jobind").val() == 3 || $("#jobind").val() == "") {
        $(".occup_tr, .15up_tr").hide();
        $(".job_inputs, .job_inputs_15up").val("").notRequired();

        if (age >= 15) {
            $(".fjob_tr, .15upnojob_tr").show();
            $("#fjob").required();

            if ($("#fjob").val() == 1) {
                $(".jsearch_meth_tr").show();
                $("#jsearch_meth").required();

                $(".ynotlookjob_tr, .ynotlookjob_o_tr").hide();
                $("#ynotlookjob, #ynotlookjob_o").val("").notRequired();
            } else if ($("#fjob").val() == 2) {
                $(".jsearch_meth_tr, .jsearch_meth_o_tr").hide();
                $("#jsearch_meth, #jsearch_meth_o").val("").notRequired();

                $(".ynotlookjob_tr").show();
                $("#ynotlookjob").required();
            } else {
                $(".ynotlookjob_tr, .ynotlookjob_o_tr").hide();
                $("#ynotlookjob, #ynotlookjob_o").val("").notRequired();
    ...
    if ((numpwd > 0 && numpwd != pwds) || (numpwd > 0 && memIsPWD)) {
        $(".pwd").show();
        $("#pwdind").required();
    } else {
...
$("#addmemModal").on("shown.bs.modal", function (e) {
    ...
    $("#fcourse").autocomplete({
        source: psced7,

        select: function (event, ui) {
            $("#fpsced7").val(ui.item.idx);
        }
    });
...
$("#addmemModal").on("hidden.bs.modal", function (e) {
    ...
    $("#psoc4_label, #psic4_label, #course, #fcourse").autocomplete(
        "destroy");
    document.getElementById("addmem_form").reset();
});
...
function edit_deceased(deceasedToEdit) {
    ...
    $.each(hpq_death_TEMP, function (i, deceasedRow) {
        ...
        if (deceasedRow.dindex == deceasedToEdit) {
            for (let key in deceasedRow) {

                $("#adddeceased_form").find("#" + key).val(deceasedRow[key]);
